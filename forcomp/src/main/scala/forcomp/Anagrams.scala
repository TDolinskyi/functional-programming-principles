...
object Anagrams extends AnagramsInterface:
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    @tailrec
    def _combination(occ: Occurrences, result: Set[Occurrences]): List[Occurrences] = {
      if (occ.nonEmpty) {
        val combination = occ match
          case (char, count) :: _ =>
            for {
              current <- result
              newOcc <- 1 to count
            } yield {
              if (current.exists((ch, _) => ch == char)) {
                Nil
              } else {
                (current :+ (char -> newOcc)).sortBy((char, _) => char)
              }

            }
          case Nil => Nil
        _combination(occ.tail, result ++ combination)
      } else {
        result.toList
      }
    ...
    val allSingles: List[Occurrences] = for {
    ...
    _combination(occurrences, allSingles.toSet) :+ Nil
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foldLeft(x.toMap) {
      case (result, (char, count)) =>
        val newCount = result(char) - count
        if (newCount == 0) result - char
        else result updated (char, newCount)
    }.toList.sortBy(_._1)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def _iter(sentenceOccur: Occurrences): List[Sentence] = {
      if (sentenceOccur.isEmpty) {
        List(Nil)
      } else for {
        comb <- combinations(sentenceOccur)
        word <- dictionaryByOccurrences getOrElse(comb, Nil)
        res <- _iter(subtract(sentenceOccur, wordOccurrences(word)))
      } yield word :: res
    }

    _iter(sentenceOccurrences(sentence))
  }
