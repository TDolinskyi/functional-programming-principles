...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted(acc: Tweet): Tweet = {
    left.mostRetweeted(right.mostRetweeted(if (elem.retweets > acc.retweets) elem else acc))
  }
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = allTweets.filter(p => google.exists(g => p.text.contains(g)))
  lazy val appleTweets: TweetSet = allTweets.filter(p => apple.exists(g => p.text.contains(g)))
